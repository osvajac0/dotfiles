...
static const int sidepad                 = 320;  /* horizontal padding of bar */
...
static const char *fonts[]   = { 
       // "LiterationMono Nerd Font:size=10:style=Bold",
        "IBM DOS VGA 9x16:size=10",
 		"FontAwesome:size=9", 
};
...
#if BAR_ALPHA_PATCH
static const unsigned int baralpha = 220; // transparency of the bar
static const unsigned int borderalpha = 220; // transparency of borders
...
static const char *const autostart[] = {
	"picom", NULL,
	"/home/pax/.fehbg", "-c", NULL, // replace "username" with your username
	"slstatus", NULL,
	"dunst", NULL,
	"/home/pax/scripts/mousepad", "-c", NULL, // replace "username" with your username 
	NULL /* terminate */
};
...
//  [DEFAULT_TAGS]        = { "добар", "дан", "како", "сте", "пријатељи", "леп", "је", "дан", "учим" },
...
static const char *mutecmd[] = { "sh", "-c",
    ...
    "if [ \"$MUTED\" = \"yes\" ]; then "
    "notify-send -r 1001 'Audio Muted' 'Sound is muted'; "
    "else "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -r 1001 'Audio Unmuted' \"$VOL\"; "
    "fi", NULL };

static const char *volupcmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 +5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -r 1002 'Volume increased' \"$VOL\"", NULL };

static const char *voldowncmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 -5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -r 1002 'Volume decreased' \"$VOL\"", NULL };
